...
namespace QueryBuilder
{
    public class QueryBuilder : IDisposable
    {
        ...
        public List<T> ReadAll<T>() where T : IClassModel, new()
        {
            ...
            while(reader.Read())
            {
                ...
                for(int i = 0; i < reader.FieldCount; i++)
                {
                    ...
                    {
                        objProperties[i].SetValue(data, Convert.ToInt32(reader.GetValue(i)));
                    }
                    // if datetime, format the string correctly
                    else if (colType == typeof(DateTime) && reader.GetValue(i).ToString().Split('-').Length == 3)
                    {
                        string[] dateString = reader.GetValue(i).ToString().Split('-');
                        int[] dateNum = new int[3];
                        for (int k = 0; k < 3; k++)
                        ...
                        var dateTime = new DateTime(dateNum[0], dateNum[1], dateNum[2]);
                        objProperties[i].SetValue(data, dateTime);
                    }
                    // other data types
                    else
                    {
                        objProperties[i].SetValue(data, reader.GetValue(i));
                    }
                }
                dataList.Add(data);
            }
        ...
        public void Create<T>(T obj) where T : IClassModel
        {
            ...
            var command = connection.CreateCommand();
            command.CommandText = $"INSERT INTO {typeof(T).Name} {sbCols} VALUES{sbVals}";
            var reader = command.ExecuteNonQuery();
        }

        public void Create<T>(List<T> collection) where T : IClassModel
        {
